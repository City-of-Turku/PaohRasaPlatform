...
DEFAULT_NLU_FALLBACK_INTENT_NAME = "get_started"

logger = logging.getLogger(__name__)


class FallbackServiceSearchClassifier(IntentClassifier):
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        if not self._should_fallback(message):
            return
        if FALLBACK_INTENT_NAME_KEY in self.component_config:
            self.fallback_intent = self.component_config[FALLBACK_INTENT_NAME_KEY]
        else:
            self.fallback_intent = DEFAULT_NLU_FALLBACK_INTENT_NAME
        ...
        message.data[INTENT] = self._fallback_intent(confidence)
        message.data.setdefault(INTENT_RANKING_KEY, [])
        message.data[INTENT_RANKING_KEY].insert(0, self._fallback_intent(confidence))
    ...
    def _should_fallback(self, message: Message) -> bool:
        ...
        if ambiguous_prediction:
            logger.debug(
                ...
                f"intent '{self.fallback_intent}' instead of "
                f"'{intent_name}'."
            )
            print(
                f"The difference in NLU confidences " +
    ...
    def _fallback_intent(self, confidence: float) -> Dict[Text, Union[Text, float]]:
        return {
            INTENT_NAME_KEY: self.fallback_intent,
