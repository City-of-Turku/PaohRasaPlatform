"""Copied from Botfront tracker store with a modification to use text_anonymizer"""
...
class BotfrontAnonymizedTrackerStore(TrackerStore):
    def __init__(self, domain, host, **kwargs):
        ...
        self.test_tracker_persist_time = kwargs.get(
            "test_tracker_persist_time", 240)
        ...
        super(BotfrontAnonymizedTrackerStore, self).__init__(
            domain, event_broker=kwargs.get("event_broker"))
    ...
    def _anonymize_tracker(self, serialized_tracker: dict) -> dict:
        serialized_tracker["latest_message"]["text"] = self.text_anonymizer.anonymize_text(
            serialized_tracker["latest_message"]["text"])
        for event in serialized_tracker["events"]:
            if event["event"] == "user":
                event["text"] = self.text_anonymizer.anonymize_text(
                    event["text"])
                event["parse_data"]["text"] = self.text_anonymizer.anonymize_text(
    ...
    def save(self, canonical_tracker):
        ...
        if tracker is None:  # the tracker does not exist localy ( first save)
            updated_info = self._insert_tracker_gql(
                sender_id, serialized_tracker)
            ...
            tracker_shallow_copy = {key: val for key,
                                    val in serialized_tracker.items()}
            tracker_shallow_copy["events"] = new_events
            # only send the new events to the remote tracker
            updated_info = self._update_tracker_gql(
                sender_id, tracker_shallow_copy)
            # update the last index and last time stamp for future uses
            self._store_tracker_info(sender_id, updated_info)
            self.trackers[sender_id] = serialized_tracker
            return serialized_tracker["events"]
    ...
    def retrieve(self, sender_id):
        ...
        if new_tracker_info is not None:
            ...
            tracker = self._update_tracker(
                sender_id, new_tracker_info.get("tracker"))
        ...
        for key in list(
            ...
            try:
                ...
                try:
                    ...
                    latest_event = tracker.get(
                        "latest_event_time", float("inf"))
                    pass
                if latest_event < max_event_time:
                    logger.debug(
                        "SWEEPER: Removing botfront test tracker {}".format(key))
                    if key in trackers:
                        del trackers[key]
    ...
    def sweep(self):
        self.cleanup_trackers(self.test_trackers,
                              self.test_tracker_persist_time)
