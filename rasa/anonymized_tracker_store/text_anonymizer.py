...
class TextAnonymizer():
    ...
    def __init__(self) -> None:
        ...
        # list of regex patterns for things we don't want to anonymize from text
        # (e.g. dates)
        re_keep_pattern_list = ['\d{4}-\d{2}-\d{2}']
        self.re_keep_pattern = re.compile(
            '(' + '|'.join(re_keep_pattern_list) + ')')

    def _replace_phone(self, match: re.Match, min_digits: int = 6) -> str:
    ...
    def anonymize_text(self, text: str) -> str:
        ...
        for index, item in enumerate(splitted_text):
            if re.match(self.re_keep_pattern, item) is None:
                ...
                item = re.sub(self.phone, self._replace_phone, item)
